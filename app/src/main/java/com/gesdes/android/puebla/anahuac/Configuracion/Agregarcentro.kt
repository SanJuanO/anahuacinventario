...
class Agregarcentro : AppCompatActivity() {
    ...
    //    val im = findViewById<View>(R.id.atras) as ImageButton
      //  im.setOnClickListener { finish() }
        URL = getString(R.string.URL_API_CENTRO) + "addCentroCostos"
        val preferences = this.getSharedPreferences("VARIABLES", Context.MODE_PRIVATE)

        val temp = preferences.getBoolean("PSCC", false)
        ...
        if (idbuscar != "") {
            if (temp) {
                ...
                tit.setText("Detalle de Centro de Costos");
        ...
        val jsonObjectRequest: JsonObjectRequest = object : JsonObjectRequest(Method.POST, "https://uap-inventarios-wa.azurewebsites.net/api/CentrosCostosApi/saveCentroCostos", datos,
    ...
    fun consultar() {
        ...
        val requstQueue = Volley.newRequestQueue(this)
        val jsonObjectRequest: JsonObjectRequest = object : JsonObjectRequest(Method.POST, "https://uap-inventarios-wa.azurewebsites.net/api/CentrosCostosApi/getCentroCostosById", datos,
                Response.Listener { response ->
                    try {
                        progressDialog?.dismiss()
                        val result = response["resultado"] as Int
                        if (result == 1) {
                            ...
                            nomcosto.setText(costos.getString("centroCostos"))
